...
char* text;
{
    ...
    type_name = (type == MSG_USR) ? "USER" : (type == MSG_AST) ? "ASSISTANT" : "UNKNOWN";
...
int ch_api()
{
    ...
    */
    ...
    /* Send content length first (16-bit, low byte then high byte) */
    outp(126, reqlen & 0xFF);
    outp(127, (reqlen >> 8) & 0xFF);

    /* Trigger the API call - Core 1 will send HTTP headers */
    inp(120);

    /* Send JSON payload in 256-byte chunks */
    i = 0;
    while (i < reqlen)
    {
        /* Check status port 123 - if 4 (BUSY), wait */
        /* This prevents overflowing the Pico's input buffers */
        while (inp(123) == 4)
            ;
    ...
    /* Send null terminator to signal end of body */
    outp(121, 0);
...
int echo;
{
    ...
    /* CRITICAL: Must keep reading even when buffer full to prevent deadlock */
    while (1)
    {
        ...
        { /* OPENAI_DATA_READY */
            ...
            while (status == 2)
            {
                ...
                {
                    /* Buffer full - discard overflow but keep reading */
                    overflow_count++;
                }
    ...
    if (overflow_count > 0)
    {
        printf("\n[Warning: Response truncated, %d bytes discarded]\n", overflow_count);
    }
